      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed right-0 top-0 bottom-0 w-full max-w-md bg-[#111] z-50 flex flex-col"
          >
            {/* Header */}
            <div className="flex items-center justify-between p-6 border-b border-white/10">
              <div className="flex items-center gap-3">
                <ShoppingBag className="w-5 h-5 text-[#C9A962]" />
                <h2 className="text-white text-lg font-light tracking-wide">Your Cart</h2>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              {cart.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center">
                  ...
                  <button
                    onClick={onClose}
                    className="mt-4 text-[#C9A962] text-xs tracking-[0.15em] uppercase hover:underline"
                  >
                    Continue Shopping
                  </button>
                </div>
                ...
                cart.map((item, index) => (
                  <div key={index} className="flex gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[10px] text-[#C9A962] tracking-[0.2em] uppercase">{item.brand}</p>
                      <p className="text-white text-sm font-light truncate">{item.name}</p>
                      <p className="text-white/60 text-sm mt-1">${item.price?.toLocaleString()}</p>
            ...
            {cart.length > 0 && (
              <div className="p-6 border-t border-white/10 space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-white text-lg font-light">${total.toLocaleString()}</span>
                </div>
                <Link
                  to={createPageUrl("Checkout")}
                  onClick={onClose}
                  className="block w-full bg-[#C9A962] text-[#0A0A0A] text-center py-4 text-xs tracking-[0.15em] uppercase font-medium hover:bg-[#D4B870] transition-colors"
                >
                  Proceed to Checkout
                </Link>
                <button
                  onClick={onClose}
                  className="block w-full text-center text-white/40 text-xs tracking-[0.1em] uppercase hover:text-white/60 transition-colors"
                >
                  Continue Shopping
                </button>
              </div>
